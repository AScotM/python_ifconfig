...
# Optional: filter only specific interfaces (set to empty to include all)
FILTERED_INTERFACES = {"eth0", "enp0s3", "wlan0"}  # Example: only show these
# FILTERED_INTERFACES = set()  # Uncomment to disable filtering
...
def display_network_info():
    ...
    for interface in interfaces:
        if FILTERED_INTERFACES and interface not in FILTERED_INTERFACES:
            continue
...
def display_traffic_stats():
    ...
    try:
        ...
        for line in lines[2:]:
            ...
            iface = parts[0].strip()
            if FILTERED_INTERFACES and iface not in FILTERED_INTERFACES:
                continue
